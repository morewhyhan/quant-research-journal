#!/usr/bin/env python3
"""Run isolated ordinary backtests; require B0 replication before comparing variants."""
import argparse
...
OUT = HERE / 'year-2025'
OLD = ROOT / 'validation/equity60_reserve40/year-2025-full/v3-server-equity60'
...
def inputs_manifest():
    ...
    paths += [HERE / 'config.backtest.json', ROOT / 'run_backtest.py', ROOT / 'fix_dns.py']
...
def check_baseline():
    ...
    if not all(checks.values()):
        raise RuntimeError(f'B0 did not reproduce the approved baseline: {checks}')
    print('BASELINE VERIFIED: metrics and all 864 entry/exit records match.', flush=True)
...
def main():
    ...
    print('ALL SIX COMPLETE; report: validation/e0v1e64/RESULTS.md', flush=True)
