#!/usr/bin/env python3
"""Summarize all six actual exports with a strict twelve-month accounting window."""
...
VERSIONS = ('B0', 'V1', 'V2', 'V3', 'V4', 'V5')
LABELS = {'B0':'原样基准', 'V1':'空单退出校正', 'V2':'亏损超时退出',
          'V3':'预留资金条件投入', 'V4':'信号仓位系数', 'V5':'盈利持仓延长'}
...
def main():
    ...
    old = {key(t):t for t in records['B0']}
    for version in VERSIONS:
    ...
    lines = ['# E0V1E64：2025全年基准与V1～V5回测结果','',
        '六个独立进程、同一198项配置白名单（196项有5m数据）、8000 USDT、单持仓、5m普通回测、单边手续费0.05%、相同离线市场模拟。',
        'B0复现检查：'+('全部通过，864笔进出记录与之前动态仓位基准一致。' if all(validation.values()) else '未通过！'),'',
    ...
    if len(improved) == 1:
        ...
        lines += ['', f'本轮只有{winner}同时超过B0的全年收益和金额PF，列为下一阶段研究候选。',
    ...
    for m in range(12):
        lines.append(f'| 2025-{m+1:02d} | '+' | '.join(f'{details[v]["monthly"][m]["return_pct"]:+.2f}%'
                     for v in VERSIONS)+' |')
    lines += ['', '## 新增退出实际触发情况','']
    for version in ('V1','V2','V5'):
        for reason, item in details[version]['groups']['exit_reason'].items():
            if reason.startswith(version.lower()+'_'):
