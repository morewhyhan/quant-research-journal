"""Serial, independently funded July cross-check of every frozen candidate."""
import argparse
...
OUT=HERE/'july-2026'
DATA=ROOT/'user_data/data/okx'


def fingerprint():
    files=inputs_manifest()
    files.update({str(p.relative_to(ROOT)):sha(p) for p in (HERE/'JULY_PLAN.md',Path(__file__))})
    return files
...
def build_manifest():
    ...
    return {'created_utc':datetime.now(timezone.utc).isoformat(),
        'timerange':'20260701-20260801','data_directory':str(DATA.relative_to(ROOT)),
...
def run_one(version, manifest):
    ...
    if (folder/'completed.json').exists():
        ...
        print('RESUME JULY '+version,flush=True)
        return
    ...
    cmd=[sys.executable,str(ROOT/'run_backtest.py'),'backtesting',
        ...
        '--timerange','20260701-20260801','--fee','0.0005','--enable-protections',
        '--cache','none','--export','trades','--backtest-directory',str(folder)]
    started=time.time()
    print('START JULY '+version,flush=True)
    with (folder/'run.log').open('w') as log:
        ...
        code=process.wait()
    if code:
        raise RuntimeError(f'{version}: July backtest failed ({code}), inspect run.log')
    if fingerprint()!=manifest['input_hashes']:
        raise RuntimeError('Frozen inputs changed during July backtest')
    result,path=read_result(folder)
    ...
    print(f'DONE JULY {version}: return={result["profit_total"]*100:.4f}% '
...
def report():
    ...
    (HERE/'JULY_RESULTS.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
    ...
    with (OUT/'.runner.lock').open('w') as lock:
        ...
        if path.exists():
            ...
            if manifest['input_hashes']!=current['input_hashes'] or manifest['coverage']!=current['coverage']:
                raise RuntimeError('July inputs/data changed since prior run')
        else:
            manifest=current
        ...
        if build_manifest()['coverage']!=manifest['coverage']:
            raise RuntimeError('July data changed during testing')
        report()
        print('YEAR DIAGNOSTICS AND ALL FIVE JULY CHECKS COMPLETE',flush=True)
